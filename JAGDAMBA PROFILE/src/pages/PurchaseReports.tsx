...
export const PurchaseReports: React.FC = () => {
  ...
  // Helper to get total received for a PO
  ...
  const supplierWise = useMemo(() => {
    ...
    purchaseOrders.forEach(po => {
      ...
      map[po.supplierName].ordered += po.totalKg;
      map[po.supplierName].received += received;
      map[po.supplierName].pending += Math.max(0, po.totalKg - received);
  ...
  const itemWise = useMemo(() => {
    ...
    purchaseOrders.forEach(po => {
      po.items.forEach(item => {
        ...
        const received = 0; // Simplified for item-wise as receipt is PO-level currently
        if (!map[key]) {
          map[key] = { ordered: 0, received: 0, pending: 0 };
        }
        map[key].ordered += item.kg;
        map[key].received += received;
        map[key].pending += Math.max(0, item.kg - received);
      });
    });
    return Object.entries(map).sort((a, b) => b[1].ordered - a[1].ordered);
  }, [purchaseOrders, purchaseReceipts]);
  ...
  const handleExport = () => {
    // Fake export
    const csvContent = "PO Number,Supplier,Total Kg,Received,Pending,Status\n" + 
      filteredOrders.map(o => {
        const received = getReceivedQty(o.id);
        return `${o.poNumber},${o.supplierName},${o.totalKg},${received},${o.totalKg - received},${o.status}`;
      }).join("\n");
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Ordered (MT)</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Received (MT)</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Pending (MT)</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredOrders.length === 0 ? (
                  ...
                  const received = getReceivedQty(po.id);
                  const pending = po.totalKg - received;
                  ...
                  return (
                    <React.Fragment key={po.id}>
                      ...
                      <td className="p-4 text-sm text-slate-800 text-right font-medium">{po.totalKg.toFixed(3)}</td>
                      <td className="p-4 text-sm text-emerald-600 text-right font-medium">{received.toFixed(3)}</td>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Total Ordered (MT)</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Total Received (MT)</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Total Pending (MT)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {supplierWise.map(([supplier, data]) => (
                  <tr key={supplier} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-slate-800 text-right font-medium">{data.ordered.toFixed(3)}</td>
                    <td className="p-4 text-sm text-emerald-600 text-right font-medium">{data.received.toFixed(3)}</td>
                    <td className="p-4 text-sm text-blue-600 text-right font-bold">{data.pending.toFixed(3)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Ordered (MT)</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Received (MT)</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">In Transit / Pending (MT)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
