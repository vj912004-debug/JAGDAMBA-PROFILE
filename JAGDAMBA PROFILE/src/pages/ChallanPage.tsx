...
export const ChallanPage: React.FC = () => {
  ...
  const selectedOrderData = orders.find(o => o.orderNo === selectedDispatch?.orderNo);
  ...
  const updatePayment = (id: string, paid: number) => {
    if (!canEdit) { toast.error('Only Admin or Accounts can update payment'); return; }
    setChallans(prev => prev.map(c => {
      if (c.id !== id) return c;
      const newPaid = Math.min(paid, c.totalAmount);
      const newBalance = c.totalAmount - newPaid;
      return { 
        ...c, 
        amountPaid: newPaid, 
        balanceAmount: newBalance,
        status: newBalance <= 0 ? 'Done' : 'Pending'
      };
    }));
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 fade-in">
      ...
      {!canEdit && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 text-sm text-amber-800 font-medium">
      ...
      {showCreate && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-teal-100 slide-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-500 mb-1">Dispatch Record *</label>
            ...
            <div>
              ...
              <input type="number" min="0" value={taxableAmount || ''} onChange={(e) => setTaxableAmount(Number(e.target.value))} className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none" />
            </div>
            <div>
                ...
                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-3">
                    <input
                      ...
                      onChange={(e) => updatePayment(item.id, Number(e.target.value))}
                      disabled={!canEdit}
                      className="w-24 bg-white border border-slate-200 rounded px-2 py-1 text-xs focus:ring-2 focus:ring-teal-500 outline-none"
                    />
                  </td>
