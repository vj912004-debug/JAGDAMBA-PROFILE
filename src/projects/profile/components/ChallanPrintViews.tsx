...
import { LOGO_PRINT_BASE64 } from '../utils/logoPrintBase64';
import { LOGO2_BASE64 } from '../utils/logo2Base64';

export const CHALLAN_ORIGINAL_PRINT_ID = 'challan-original-print-area';
export const CHALLAN_DUPLICATE_PRINT_ID = 'challan-duplicate-print-area';

export interface ChallanPrintViewProps {
...
export function buildChallanPrintData(
  ...
  const totalKg = items.reduce(
    (sum, item) => (item.unitType === 'Kg' ? sum + item.quantity : sum + (item.totalWeight || 0)),
    0
  );
...
export const ChallanPrintStyles: React.FC = () => (
  <style>{`
    .challan-print-root, .challan-print-root * {
    ...
    @media print {
      ...
      .print-hide-duplicate { display: none !important; }
      @page {
        size: A4 portrait;
        margin: 5mm;
      }
    ...
    .grid-cell.no-right-border { border-right: none; }
    .grid-cell label { font-weight: bold; margin-right: 10px; }
    ...
    .items-table tr.empty-row td { height: 25px; }
    ...
    .signature-box:last-child { border-right: none; align-items: flex-end; }
    .sig-label-bottom { align-self: flex-end; width: 100%; text-align: right; }
    ...
    .challan-container-orig, .challan-container-orig * {
      box-sizing: border-box;
      margin: 0;
      padding: 0;
      font-family: Arial, sans-serif;
...
export const ChallanOriginalPrintView: React.FC<ChallanPrintViewProps> = ({
  challan,
  order,
  party,
  vehicleNo,
  ...
  const tcVal = order?.tc === 'Yes' ? 'YES' : '';
  const utVal = order?.ut === 'Yes' ? 'YES' : '';
  const loadingVal =
    order?.loadingUnloadingCharges && order.loadingUnloadingCharges > 0
      ? `₹${order.loadingUnloadingCharges.toLocaleString('en-IN')}`
      : '';
  const transportVal =
    order?.transportationCharges && order.transportationCharges > 0
      ? `₹${order.transportationCharges.toLocaleString('en-IN')}`
      : '';

  return (
    <div className="challan-container-orig" id={CHALLAN_ORIGINAL_PRINT_ID}>
      ...
      <div className="inner-border-orig">
        <div className="header-orig">
          <div className="logo-container-orig">
            <img src={LOGO_PRINT_BASE64} alt="Jagdamba Profile Logo" className="company-logo-orig" />
        ...
        <div className="signature-row-orig">
          <div className="sig-box-orig"><div>Receiver&apos;s Signature</div></div>
          <div className="sig-box-orig"><div>Dispatch</div></div>
...
export const ChallanDuplicatePrintView: React.FC<ChallanPrintViewProps & { className?: string }> = ({
  challan,
  order,
  party,
  vehicleNo,
  ...
  <div className={`challan-container ${className ?? ''}`.trim()} id={CHALLAN_DUPLICATE_PRINT_ID}>
    <div className="inner-border" style={{ position: 'relative' }}>
      <div
        className="copy-stamp-orig"
        style={{ top: '10px', right: '10px', lineHeight: '1.2', paddingBottom: '4px', boxSizing: 'content-box' }}
      >
        DUPLICATE COPY
      </div>
      ...
      <div className="grid-2-col">
        ...
        <div className="grid-cell no-right-border">
          <label>Challan No:</label> <span style={{ color: '#1e3a8a', fontWeight: 'bold' }}>{challan.challanNo}</span>
        </div>
      ...
      <div className="commercial-grid">
        ...
        <div className="grid-cell" style={{ borderBottom: 'none', display: 'flex' }}>
          <label>TC :</label>
          <div className="underline-input">{order?.tc === 'Yes' ? 'YES' : ''}</div>
        </div>
        <div className="grid-cell no-right-border" style={{ borderBottom: 'none', display: 'flex' }}>
          <label>UT :</label>
          <div className="underline-input">{order?.ut === 'Yes' ? 'YES' : ''}</div>
        </div>
      </div>
      ...
      <div className="signatures-section">
        <div className="signature-box"><span>Receiver&apos;s Signature</span></div>
        <div className="signature-box"><span>Dispatch</span></div>
