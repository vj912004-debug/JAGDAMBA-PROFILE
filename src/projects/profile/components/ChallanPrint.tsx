...
export const ChallanPrint: React.FC<ChallanPrintProps> = ({ challan, order }) => {
  ...
  function ChallanOriginalCopy() {
    ...
    const loadingVal = order?.loadingUnloadingCharges && order.loadingUnloadingCharges > 0 ? `₹${order.loadingUnloadingCharges.toLocaleString('en-IN')}` : '';
    const transportVal = order?.transportationCharges && order.transportationCharges > 0 ? `₹${order.transportationCharges.toLocaleString('en-IN')}` : '';
