...
export const Dashboard: React.FC = () => {
  ...
  const secondaryStats = [
    { label: 'Pending Dispatch', value: pendingDispatchCount.toString(), icon: Truck, color: 'text-blue-600 dark:text-blue-450', bg: 'bg-blue-50 dark:bg-blue-950/20', border: 'border-blue-100 dark:border-blue-900/30', route: '/dispatch' },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 fade-in transition-colors duration-300">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => navigate('/order-entry')} className="hidden sm:flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-4 py-2 rounded-xl text-sm font-semibold shadow-sm transition-all active:scale-95">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {secondaryStats.map((stat, idx) => (
          <div
            key={idx}
            onClick={() => navigate(stat.route)}
            className={`bg-white dark:bg-slate-900 rounded-xl p-3.5 shadow-sm border flex items-center gap-3 cursor-pointer hover:bg-slate-50 dark:bg-slate-800/50 dark:hover:bg-slate-800/50 transition-all ${stat.border}`}
          >
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold text-slate-500 dark:text-slate-400">{stat.label}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            {stageCounts.map((s, i) => (
              <div key={s.stage} className="flex items-center gap-1.5">
                <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-center min-w-[80px] transition-colors">
                  ...
                  <p className="text-[9px] font-semibold text-slate-500 dark:text-slate-450 uppercase tracking-wider leading-tight">{s.stage}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-all duration-300">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50/50 dark:bg-slate-900/50 flex items-center justify-between transition-colors">
            ...
            <button onClick={() => navigate('/production-status')} className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 font-semibold">View All →</button>
          </div>
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {recentOrders.map(order => (
              <div key={order.id} className="px-4 py-3 flex items-center justify-between hover:bg-slate-50 dark:bg-slate-800/50/50 dark:hover:bg-slate-800/30 transition-colors">
                ...
                <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-full whitespace-nowrap transition-colors">
                  {order.stage}
                </span>
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden transition-all duration-300">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {urgentOrders.map(order => (
              <div key={order.id} className="px-4 py-3 flex items-center justify-between hover:bg-red-50/30 dark:hover:bg-red-950/10 transition-colors">
                <div>
                  <p className="text-sm font-bold text-slate-800 dark:text-slate-100">{order.partyName}</p>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                ...
                <span className="text-[10px] font-bold text-amber-700 dark:text-amber-400 bg-amber-100 dark:bg-amber-950/40 px-2 py-1 rounded-full whitespace-nowrap transition-colors">
                  {order.stage}
                </span>
              </div>
            ))}
