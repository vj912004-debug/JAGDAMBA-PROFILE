...
export const Header: React.FC = () => {
  ...
  return (
    <header className="bg-white dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 h-14 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-10 shadow-sm transition-colors duration-300">
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-full border border-slate-200 dark:border-slate-700 transition-colors">
          ...
          <div className="flex bg-white dark:bg-slate-900 rounded-full p-0.5 shadow-sm transition-colors">
            <button
              ...
              className={`px-2.5 py-1 text-xxs font-semibold rounded-full transition-all ${language === 'English' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:hover:bg-slate-800'}`}
            >
              EN
            </button>
            <button
              onClick={() => setLanguage('Gujarati')}
              className={`px-2.5 py-1 text-xxs font-semibold rounded-full transition-all ${language === 'Gujarati' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:hover:bg-slate-800'}`}
            >
              ગુજ
            </button>
          </div>
        ...
        <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors">
          <div className="hidden sm:flex flex-col items-end mr-1">
            <span className="text-xxs font-bold text-slate-800 dark:text-slate-200 leading-tight">{user?.displayName}</span>
            <span className="text-3xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-tighter leading-tight">{user?.role}</span>
          </div>
          <div className="h-6 w-px bg-slate-300 dark:bg-slate-700 mx-1 hidden sm:block"></div>
          <button
            onClick={logout}
            className="p-1 text-slate-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-400 transition-colors"
            title="Logout"
          >
