        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-blue-100 dark:border-blue-800 slide-up">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div>
              ...
              <input type="text" value={newPlate.id} onChange={e => setNewPlate({ ...newPlate, id: upper(e.target.value) })} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="PLT-1005" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="text" value={newPlate.thickness} onChange={e => setNewPlate({ ...newPlate, thickness: upper(e.target.value) })} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="10mm" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Size (W x L)</label>
              <input type="text" value={newPlate.size} onChange={e => setNewPlate({ ...newPlate, size: upper(e.target.value) })} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="1250 x 2500" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Heat/TC Number</label>
              <input type="text" value={newPlate.heatNumber} onChange={e => setNewPlate({ ...newPlate, heatNumber: upper(e.target.value) })} className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="TC-89238" />
            </div>
            <div>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                <th className="p-3 table-head-cell">Plate ID</th>
                <th className="p-3 table-head-cell">Grade / Thickness</th>
                <th className="p-3 table-head-cell">Size</th>
                <th className="p-3 table-head-cell">Heat/TC No</th>
                <th className="p-3 table-head-cell">Source</th>
                <th className="p-3 table-head-cell">Initial (Kg)</th>
                <th className="p-3 table-head-cell">Used (Kg)</th>
                <th className="p-3 text-2xs font-bold tracking-wider uppercase text-red-600 dark:text-red-400">Scrap (Kg)</th>
                <th className="p-3 text-2xs font-bold tracking-wider uppercase text-emerald-600 dark:text-emerald-400">Balance (Kg)</th>
                <th className="p-3 table-head-cell">Orders</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredPlates.map(plate => {
                ...
                return (
                  <tr key={plate.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 text-2xs font-bold rounded-full ${plate.source === 'Customer' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300'}`}>
                        {plate.source}
                      </span>
                    </td>
                    ...
                    <td className="p-3 text-sm text-slate-600 dark:text-slate-300">
                      {stats.usageRecords.map(u => (
                        <span key={u.id} className="inline-block bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-2xs font-bold px-1.5 py-0.5 rounded mr-1">{u.orderNo}</span>
                      ))}
